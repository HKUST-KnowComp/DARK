...
def ppo_train_loop(args, model, ppo_trainer, tokenizer, graph_samplers, is_gpt, is_act, src_len, tgt_len):
    print('Start PPO Tuning with latest TRL version')
    
    # 设置生成参数
    generation_kwargs = {
        "min_length": -1,
        "top_k": args.ppo_top_k,
        "top_p": 1.0,
        'max_length': tgt_len + src_len * (is_gpt == True),
        'pad_token_id': tokenizer.pad_token_id,
        'bos_token_id': tokenizer.bos_token_id,
    ...
    # 如果有约束生成，添加prefix_allowed_tokens_fn
    if is_act and args.constrained:
        prefix_allowed_tokens_fn = Prefix_allowed_tokens_fn(
            offset=offset, nentity=nentity, nrelation=nrelation, 
            special_tokens=special_tokens, tokenizer=tokenizer
        )
        generation_kwargs['prefix_allowed_tokens_fn'] = prefix_allowed_tokens_fn

    # 新版本trl使用train()方法而不是手动循环
    # 在trl 0.16.0中，我们需要手动实现训练循环，因为奖励计算需要特殊的逻辑
    
    print(f"Starting PPO training with {len(ppo_trainer.train_dataset)} samples")
    
    # 设置训练参数
    batch_size = ppo_trainer.args.per_device_train_batch_size
    niter = len(ppo_trainer.train_dataset) // batch_size
    
    # 手动训练循环，参考mdm.py的实现
    from tqdm import tqdm
    import warnings
    from akgr.evaluation import scoring_input_act_batch, scoring_input_wordlist_batch
    ...
    for iter, batch in (pbar := tqdm(enumerate(ppo_trainer.dataloader, start=1),
        ...
        # 使用正确的评分函数计算jaccard index
        scoring_fn = scoring_input_act_batch if is_act else scoring_input_wordlist_batch
        scores, failures_batch_id = scoring_fn(
            pred_word_batch=pred_tgt_decoded,
            label_word_batch=target,
            ans_word_batch=source,
            scoring_method=['jaccard'],  # 只使用jaccard
            do_correction=args.do_correction,
            graph_samplers=graph_samplers,
        ...
        # 计算奖励：直接使用jaccard分数
        rewards = [torch.tensor(score['jaccard'], dtype=torch.float, device=ppo_trainer.current_device) for score in scores]
        ...
        pbar.set_description(f'r: {round(rewards_mean.item(), 6)}, '
...
def optimize(args, dataset, model, tokenizer, graph_samplers, batch_size,
    ...
    ppo_config = PPOConfig(
        seed=42,  # 默认种子
        num_ppo_epochs=args.ppo_epochs,  # 新版本使用num_ppo_epochs
        learning_rate=args.ppo_lr,
        kl_coef=args.ppo_init_kl_coef,  # 新版本使用kl_coef
        cliprange=args.ppo_cliprange,
        gamma=0.99,
        per_device_train_batch_size=batch_size,  # 新版本使用per_device_train_batch_size
        mini_batch_size=args.ppo_minibatch,
        remove_unused_columns=False,  # 重要：默认会移除未识别的列
        lr_scheduler_type="cosine",  # 新版本使用lr_scheduler_type
        warmup_ratio=0.1,  # 预热比例
        warmup_steps=int(len(dataset) * 0.1),  # 预热步数
        lr_scheduler_kwargs={
            "eta_min": args.ppo_lr * 0.1,  # 最小学习率
        },
        # log_with='wandb',  # 可选：wandb日志
    )
    print(ppo_config)
    
    # 新版本trl不需要手动创建optimizer和scheduler，PPOTrainer会自动处理
    # 在trl 0.16.0中，我们可以使用自定义奖励函数，不需要预训练的奖励模型
    # 创建一个简单的自定义奖励函数
    
    def custom_reward_function(queries, responses, **kwargs):
        """自定义奖励函数，使用jaccard index"""
        # 这里我们会在训练循环中手动计算奖励
        # 暂时返回固定值，实际奖励会在step中计算
        return [0.0] * len(queries)
    
    ppo_trainer = PPOTrainer(
        args=ppo_config,  # 新版本使用args参数
        model=model, 
        ref_model=ref_model,
        reward_model=custom_reward_function,  # 使用自定义奖励函数
...
def load_model_by_mode(args, device, model_name, is_gpt, config_model=None, ntoken=None, config_train=None):
    if args.mode in ['training', 'testing', 'optimizing'] and args.resume_epoch != 0:
        ...
        # 添加transformers版本兼容性处理
        # model = patch_model_for_transformers_compatibility(model)
    ...
    if args.mode in ['optimizing', 'testing'] and args.ppo_resume_epoch != 0: # Load TRL model wrapper directly
        ...
        if args.ppo_use_peft:
            ...
            try:
                ...
                print(f"TRL加载失败，尝试兼容性加载: {e}")
                # model = load_compatible_trl_model(resume_dir, trl_model_class, is_gpt)
...
# 删除patch_model_for_transformers_compatibility和load_compatible_trl_model相关内容
