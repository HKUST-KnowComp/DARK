...
class DiffuGRPOTrainer(GRPOTrainer):
    ...
    def __init__(
        ...
        print(f'Model config type: {type(model.config) if hasattr(model, "config") else "No config"}')
        if hasattr(model, 'config'):
            print(f'Model config._name_or_path: "{model.config._name_or_path}"')
            print(f'Model config attributes: {list(model.config.__dict__.keys())[:10]}...')  # 只显示前10个
        print('================================')
        
        # 禁用 safetensors 保存，使用 torch.save
        self.args.save_safetensors = False
        
        # # 处理共享张量问题
        # def _save_model(self, output_dir: str, **kwargs):
        #     """重写保存方法，处理共享张量"""
        #     # 创建模型的副本，避免共享张量问题
        ...
        # # 替换保存方法
        # self._save_model = _save_model.__get__(self)
    ...
    def _get_per_token_logps(self, model, input_ids, logits_to_keep, mask_seeds):
        ...
        for iter_idx, mask_seed in enumerate(mask_seeds):
            ...
            perturbed_seq, t_weights, completion_mask = self.forward_process(
                expanded_input, prompt_index, model.mask_index, seed=mask_seed, accumulate=num_iterations > 1
            )
    ...
    def unify_tokenizer(self, sources, targets, strategy = 'abductive'):
        self.processing_class.padding_side = 'left'
        source_with_start = ['START ' + s for s in sources]
        source_tokenized = self.processing_class(
            source_with_start,
            padding='max_length',
            max_length=self.args.src_len + 1,  # +1 for START token
            add_special_tokens=False,
            return_tensors="pt")
        
        # Step 2: Create tgt + [START] + padding (left padding)
        self.processing_class.padding_side = 'right'
        target_with_start = [t + 'START' for t in targets]
        target_tokenized = self.processing_class(
            target_with_start,
            padding='max_length',
            max_length=self.args.tgt_len + 1,  # +1 for START token
            add_special_tokens=False,
            return_tensors="pt")
        ...
        sep_id = torch.full((batch_size, 1), self.processing_class.sep_token_id)
        sep_mask = torch.ones((batch_size, 1))
        
        input_ids = torch.cat([source_ids, sep_id, target_ids], dim=1)
        if strategy == 'abductive':
            # For abductive: only keep source_mask and sep_mask, set target_mask to 0
            target_mask_zero = torch.zeros_like(target_mask)
            attention_mask = torch.cat([source_mask, sep_mask, target_mask_zero], dim=1)
        elif strategy == 'deductive':
            # For deductive: only keep sep_mask and target_mask, set source_mask to 0
            source_mask_zero = torch.zeros_like(source_mask)
            attention_mask = torch.cat([source_mask_zero, sep_mask, target_mask], dim=1)
        else:
            # Default case: keep all masks
    ...
    def _generate_and_score_completions(
        ...
        sources = [x['source'] for x in inputs]
        targets = [x['target'] for x in inputs]
        prompt_inputs = self.unify_tokenizer(sources, targets)
        # used to apply device
    ...
    # need to test
        ...
        prompt_length = self.args.src_len + 2
        ...
        logits_to_keep = self.args.tgt_len + 1
        ...
        rewards_per_func = torch.zeros(len(sources), len(self.reward_funcs), device=device)
        for i, (reward_func, reward_processing_class) in enumerate(
            zip(self.reward_funcs, self.reward_processing_classes)
        ):
            if isinstance(reward_func, nn.Module):
                reward_func_name = f"reward {reward_func.config._name_or_path.split('/')[-1]}"
            else: #directly use function name
        ...
        process_slice = slice(
            self.accelerator.process_index * len(sources),
            (self.accelerator.process_index + 1) * len(sources),
        )
