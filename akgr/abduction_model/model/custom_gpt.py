...
try:
    ...
    from transformers import CausalLMOutput
import torch
...
class DiffusionModel(GPT2LMHeadModel):
    """
    Diffusion model based on GPT2 architecture
    """
    
    def __init__(self, config):
        # Initialize the parent GPT2LMHeadModel with the config
        super().__init__(config)
        
        # Remove causal mask for diffusion (allow bidirectional attention)
        for gpt2block in self.transformer.h:
            gpt2block.attn.bias.fill_(True)
        
        # Disable causal attention in config for diffusion model
    ...
    def forward(
        ...
        # Set default attention mask if not provided
        if attention_mask is None:
            attention_mask = torch.ones_like(input_ids)
        
        # For diffusion model, we need to disable causal attention
        # Set use_cache to False to avoid causal mask creation
        use_cache = False
        
        # Forward through transformer with custom attention handling
        ...
        # Return standard CausalLMOutput object
    ...
    def _sample(
        ...
        for i in range(2 * max_gen_len + 2 * expand_budget):
            ...
            # here, we use 1 to pad the attention mask, because this part is also used to be train
...
def create_gpt2_transformer(ntoken: int, tokenizer,
        model_name: str):
    ...
    config = GPT2Config.from_pretrained(
            '/home/zhangziwei/gys/AbductiveKGR/hug_model',
            **common_config
        )
   
    # Create DiffusionModel directly instead of GPT2LMHeadModel
    transformer = DiffusionModel(config)
