...
class DiffuGRPOTrainer(GRPOTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        mask_attention = inputs["mask_attention"]
        ...
        per_token_logps = self._get_per_token_logps(model, input_ids, mask_attention, [this_itr_mask_seed])
    ...
    def forward_process(self, batch, mask_target, mask_id, seed=None, accumulate=False):
        set_seed(seed)
        b, l = batch.shape
        noisy_batch = []
        ...
        # qm = query_mask.bool()
        # B, L = qm.shape
        # last_idx = (L - 1) - torch.flip(qm, dims=[1]).int().argmax(dim=1)
        # rows = torch.arange(B, device=qm.device)
        # protect_mask = torch.zeros_like(qm)
        # protect_mask[:, 0] = True
        # protect_mask[rows, last_idx] = True

        # 确保query_mask是布尔类型
        query_mask = mask_target.bool()
        
        # 1. always mask completion tokens
        noisy_batch.append(torch.where(query_mask, mask_id, batch))
        
        
        # 2. mask completion tokens with probability t_p
        is_mask = (random_matrix < t_p.unsqueeze(1)) & query_mask
    ...
    def explore_process(self, batch):
        b, l = batch.shape
    
        # 计算需要mask的数量（30%）
        num_to_mask = int(l * 0.3)
    
        # 创建随机mask矩阵
        mask_matrix = torch.zeros(b, l, dtype=torch.bool)
    
        # 对每一行随机选择num_to_mask个位置设为False
        for i in range(b):
            mask_indices = torch.randperm(l)[:num_to_mask]
            mask_matrix[i, mask_indices] = True
    
        # 应用mask
        masked_batch = batch.clone()
        masked_batch[mask_matrix] = self.processing_class.mask_token_id  # 将mask位置设为mask_id
        mask_matrix_int = mask_matrix.long()  # True->1, False->0
        return masked_batch, mask_matrix_int
    ...
    def _get_per_token_logps(self, model, input_ids, mask_attention, mask_seeds):
        """
        ...
        for iter_idx, mask_seed in enumerate(mask_seeds):
            ...
            # # 找到sep_token_id的位置（处理2D张量）
            # sep_token_id = self.processing_class.sep_token_id
            # is_sep = expanded_input == sep_token_id
            # sep_idx = torch.full((is_sep.size(0),), is_sep.size(1), dtype=torch.long, device=device)
            # sep_idx[is_sep.any(dim=1)] = is_sep.int().argmax(dim=1)[is_sep.any(dim=1)]
            # sequence_indices = torch.arange(is_sep.size(1), device=device).expand(is_sep.size(0), -1)
            # sep_mask = (sequence_indices <= sep_idx.unsqueeze(1)).int()
            mask_target = mask_attention[iter_idx]
            perturbed_seq, t_weights, completion_mask = self.forward_process(
                expanded_input, mask_target, self.processing_class.mask_token_id, seed=mask_seed, accumulate=num_iterations > 1
            )
    ...
    def batch_decode(self, prompt_completion_ids):
        sep_token_id = self.processing_class.sep_token_id
         # 找到sep_token_id的位置（批量处理）
        sep_positions = (prompt_completion_ids[0] == sep_token_id).nonzero(as_tuple=True)[0]  # 取第一行找sep位置

        
        sep_pos = sep_positions[0].item()
            # 批量切片：所有样本都在同一列分割
        left_parts = prompt_completion_ids[:, :sep_pos]  # 所有行的左半部分
        right_parts = prompt_completion_ids[:, sep_pos+1:]  # 所有行的右半部分
            
        # 批量解码
        query_decodes = [self.processing_class.decode(part, skip_special_tokens=True) for part in left_parts]
        observation_decodes = [self.processing_class.decode(part, skip_special_tokens=True) for part in right_parts]
    
        return query_decodes, observation_decodes
    
    def unify_extract_sample_to_device(self, observation, query, model_type='unify'):
        ...
        query_tokenized = self.processing_class(
            query,
            add_special_tokens=False,
            max_length=self.args.query_len,
            padding='max_length',
            return_tensors="pt")
        
        observation_tokenized = self.processing_class(
            observation,
            add_special_tokens=False,
            max_length=self.args.answer_len,
            padding='max_length',
            return_tensors="pt")
        ...
        sep_id = torch.full((batch, 1), self.processing_class.sep_token_id)
        bos_id = torch.full((batch, 1), self.processing_class.bos_token_id)
        eos_id = torch.full((batch, 1), self.processing_class.eos_token_id)
    
        if model_type == 'sft_abd':
            mask_ids = torch.full((batch, query_length), self.processing_class.mask_token_id)
            input_ids = torch.cat([bos_id, mask_ids, sep_id, observation_ids, eos_id], dim=1)
            mask_attention = torch.cat([torch.zeros_like(bos_id), torch.ones_like(mask_ids), torch.zeros((batch, 1)), torch.zeros_like(observation_ids), torch.zeros((batch, 1))], dim=1)
        elif model_type == 'sft_ded':
            mask_ids = torch.full((batch, observation_length), self.processing_class.mask_token_id)
            input_ids = torch.cat([bos_id, query_ids, sep_id, mask_ids, eos_id], dim=1)
            mask_attention = torch.cat([torch.zeros_like(bos_id), torch.zeros_like(query_ids), torch.zeros((batch, 1)), torch.ones_like(mask_ids), torch.zeros((batch, 1))], dim=1)
        elif model_type == 'unify':
            query_mask_ids, query_mask_attention = self.explore_process(query_ids)
            observation_mask_ids, observation_mask_attention = self.explore_process(observation_ids)
            input_ids = torch.cat([bos_id, query_mask_ids, sep_id, observation_mask_ids, eos_id], dim=1)
            mask_attention = torch.cat([torch.zeros_like(bos_id), query_mask_attention, torch.zeros((batch, 1)), observation_mask_attention, torch.zeros((batch, 1))], dim=1)
        return {'input_ids': input_ids, 'mask_attention': mask_attention}
    
    def _generate_and_score_completions(
        self, inputs: dict[str, Union[torch.Tensor, Any]]
    ) -> dict[str, Union[torch.Tensor, Any]]:
    # we do not have prompt because we need to self-evolve the query and answer.
        device = self.accelerator.device
        
        prompts_query = [x["target"] for x in inputs]
        prompts_observation = [x["source"] for x in inputs]
        if torch.rand(1).item() < self.args.explore_ratio:
            type = 'unify'
        elif torch.rand(1).item() < self.args.deductive_ratio:
            type = 'sft_ded'
        else:
            type = 'sft_abd'
        prompts = self.unify_extract_sample_to_device(prompts_observation, prompts_query, model_type=type)
        # Configuration for the diffusion generation
        prompts = Trainer._prepare_inputs(self, prompts)
        prompt_ids = prompts["input_ids"]
        mask_attention = prompts["mask_attention"]
        mask_attention = F.pad(mask_attention, (0, 1), "constant", 0)
        generation_config = DreamGenerationConfig(mask_token_id=self.processing_class.mask_token_id, 
                                                pad_token_id=self.processing_class.eos_token_id, 
                                                bos_token_id=self.processing_class.bos_token_id, 
                                                eos_token_id=self.processing_class.eos_token_id,
                                                expand_token_id=self.processing_class.expand_token_id,
                                                delete_token_id=self.processing_class.eos_token_id,
                                                max_length = 46
                                                )
        with unwrap_model_for_generation(self.model_wrapped, self.accelerator) as unwrapped_model:
            ...
            for i in range(0, prompt_ids.size(0), generation_batch_size):
                ...
                batch_prompt_id = prompt_ids[i:end_idx]
                # here ,we are trying to generate  for each prompt
                batch_prompt_completion_ids = self.model.diffusion_generate(
                    inputs=batch_prompt_id,
            ...
            del batch_prompt_id, batch_prompt_completion_ids
            torch.cuda.empty_cache()
        ...
        prompt_length = 0
        completion_ids = prompt_completion_ids.clone()

        # Mask everything after the first EOS token that comes after SEP token
        
        sep_token_id = self.processing_class.sep_token_id
        eos_token_id = self.processing_class.eos_token_id
        
        # First find SEP token positions
        is_sep = completion_ids == sep_token_id
        sep_idx = torch.full((is_sep.size(0),), is_sep.size(1), dtype=torch.long, device=device)
        sep_idx[is_sep.any(dim=1)] = is_sep.int().argmax(dim=1)[is_sep.any(dim=1)]
        
        # Then find EOS token positions after SEP
        is_eos = completion_ids == eos_token_id
        eos_idx = torch.full((is_eos.size(0),), is_eos.size(1), dtype=torch.long, device=device)
        
        # Only consider EOS tokens that come after SEP
        for i in range(is_eos.size(0)):
            if is_eos[i].any():
                eos_positions = is_eos[i].nonzero(as_tuple=True)[0]
                sep_pos = sep_idx[i].item()
                # Find first EOS after SEP
                valid_eos = eos_positions[eos_positions > sep_pos]
                if len(valid_eos) > 0:
                    eos_idx[i] = valid_eos[0]
        ...
        with torch.no_grad():
            if self.num_iterations > 1:
                ...
                mask_attention_expanded = mask_attention.unsqueeze(0).expand(
                    self.num_iterations, -1, -1
                )
                old_per_token_logps = self._get_per_token_logps(
                    self.model, prompt_completion_ids_expanded, mask_attention_expanded, mask_seeds
                )
            ...
            if self.beta == 0.0:
                ...
                if self.ref_model is not None:
                    ...
                    ref_per_token_logps = self._get_per_token_logps(
                        self.ref_model, prompt_completion_ids_expanded, mask_attention_expanded, mask_seeds
                    )
                else:
        ...
        # need to fix the completions_text
        query_decode,observation_decode = self.batch_decode(completion_ids)        

        rewards_per_func = torch.zeros(len(prompts_query), len(self.reward_funcs), device=device)
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            with profiling_context(self, reward_func_name):
                ...
                keys = [key for key in inputs[0] if key not in ["source", "target"]]
                reward_kwargs = {key: [example[key] for example in inputs] for key in keys}
                output_reward_func = reward_func(
                    query_list=query_decode,
                    answer_list=observation_decode,
                    step=self._step,
                    **reward_kwargs,
                )
        ...
        if torch.isnan(rewards_per_func).all(dim=1).any():
            ...
            row_reward_kwargs["prompt"] = query_decode[nan_row_idx]
            row_reward_kwargs["completion"] = observation_decode[nan_row_idx]
            warnings.warn(
        ...
        process_slice = slice(
            self.accelerator.process_index * len(prompts_query),
            (self.accelerator.process_index + 1) * len(prompts_query),
        )
        ...
        # self._logs["query"].extend(gather_object(query_decode))
        # self._logs["observation"].extend(gather_object(observation_decode))
