...
def get_ans_score(
    ...
    check_result = check_all_parts_are_digits(label_ans)
    if check_result == 0:
        label_ans_unshifted = []
    else:
    ...
    if 'jaccard' in scoring_method:
        ...
        if U == 0:
            score_dict['jaccard'] = 0
        else:
            score_dict['jaccard'] = I / U
    if 'dice' in scoring_method:
        A = len(set(label_ans_unshifted))
        B = len(set(pred_ans_unshifted))
        score_dict['dice'] = 2.*I / (A+B)
    if 'overlap' in scoring_method:
        A = len(set(label_ans_unshifted))
        B = len(set(pred_ans_unshifted))
        score_dict['overlap'] = I / (min(A,B)+1e-5)
    if 'tanimoto' in scoring_method:
        # U = len(list(set(label_ans_unshifted).union(set(pred_ans_unshifted))))
        A = len(set(label_ans_unshifted))
        B = len(set(pred_ans_unshifted))
        score_dict['tanimoto'] = I / (A+B-I)
    if return_ans:
        return score_dict, pred_ans_unshifted
    else:
        return score_dict
    
def get_validity_score(pred,tgt):
    score_dict = {}
    pred_str = number_to_pattern(pred)
    tgt_str = number_to_pattern(tgt)
    if pred_str == tgt_str:
        score_dict['validity'] = 1.
    else:
        score_dict['validity'] = 0.
    return score_dict

def get_number_score(pred,tgt):
    score_dict = {}
    e1,p1 = number_to_epnumber(pred)
    e2,p2 = number_to_epnumber(tgt)
    if e1 == e2:
        score_dict['enumber'] = 1.
    else:
        score_dict['enumber'] = 0.
    if p1 == p2:
        score_dict['pnumber'] = 1.
    else:
        score_dict['pnumber'] = 0.
    return score_dict


def get_spec_score(pred,cond):
    score_dict = {}
    elements = pred.split()  # 按空格分割成列表
    if cond in elements:
        score_dict['spec'] = 1.
    else:
        score_dict['spec'] = 0.
...
def scoring_input_wordlist(
        pred_qry_wordlist, label_qry_wordlist, pred_qry, label_qry, ans_wordlist, condition,
    ...
    if 'dice' in scoring_method: score_dict_now['dice'] = 0
    if 'overlap' in scoring_method: score_dict_now['overlap'] = 0
    if 'tanimoto' in scoring_method: score_dict_now['tanimoto'] = 0
    if 'count0' in scoring_method: score_dict_now['count0'] = 1
    if 'validity' in scoring_method: 
        score_dict_now['validity'] = 0
        score_dict_now['enumber'] = 0
        score_dict_now['pnumber'] = 0
    if 'specific' in scoring_method:
        score_dict_now['spec']=0
    ...
    if 'precrecf1' in scoring_method or 'jaccard' in scoring_method:
        ...
        # print('')
    if 'validity' in scoring_method:
        output = get_validity_score(pred_qry,label_qry)
        score_dict = output if not return_ans else output[0]
        score_dict_now.update(score_dict)
        output = get_number_score(pred_qry,label_qry)
        score_dict = output if not return_ans else output[0]
        score_dict_now.update(score_dict)
    if 'specific' in scoring_method:
        output = get_spec_score(pred_qry,condition)
        score_dict = output
        score_dict_now.update(score_dict)
...
def scoring_input_wordlist_batch(
    ...
    if 'dice' in scoring_method: score_dict_zero['dice'] = 0
    if 'overlap' in scoring_method: score_dict_zero['overlap'] = 0
    if 'tanimoto' in scoring_method: score_dict_zero['tanimoto'] = 0
    if 'validity' in scoring_method: 
        score_dict_zero['validity'] = 0
        score_dict_zero['enumber'] = 0
        score_dict_zero['pnumber'] = 0
    if 'specific' in scoring_method:
        score_dict_zero['spec']=0
...
def scoring_input_act_batch_condition(
        pred_word_batch, label_word_batch, ans_word_batch, condition_batch,
        scoring_method: list,
        do_correction: bool=False,
        graph_samplers=None,
        searching_split=None,
    ...
    for i, (pred_act, label_act, label_ans,condition) in \
        enumerate(zip(pred_word_batch, label_word_batch, ans_word_batch, condition_batch)):
        if verbose:
        # if True:
            print('label')
            print(pred_act)
...
def check_all_parts_are_digits(ans_list):
    """检查字符串中空格分割的所有部分是否都是数字"""
    return 1 if all(isinstance(item, int) for item in ans_list) else 0


def get_reward_deduction(answer_list):
    results = [check_all_parts_are_digits(item) for item in answer_list]
    return results